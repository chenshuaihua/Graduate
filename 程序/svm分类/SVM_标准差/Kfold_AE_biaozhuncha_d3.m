...
%%%%%%%读取数据
x1 = load("E:\大学\大四学年（2019-2020）\下学期\毕设\数据\1\标准差\A_biaozhuncha_d3.txt");
x2 = load("E:\大学\大四学年（2019-2020）\下学期\毕设\数据\1\标准差\E_biaozhuncha_d3.txt");
...
for i = 1:5
    ...
    sen = (aa/(aa+cc))*100;
    spe = (dd/(bb+dd))*100;
    disp(['灵敏度为:',num2str(sen),'%'])
    disp(['特异度为:',num2str(spe),'%'])
...
%     plot(label_test,'bo');
%     title("支持向量机对AE两组的分类结果（根据CD3的标准差）")
